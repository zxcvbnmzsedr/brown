...
def record_snapshot(db: DbSession, current_user: CurrentUser):
...
def list_snapshots(
    ...
    records = db.scalars(
        ...
        .where(SnapshotHistory.user_id == current_user.id)
        .order_by(SnapshotHistory.recorded_at.desc())
        .limit(limit)
    ).all()
    return records
...
def record_rebalance(payload: RebalanceHistoryCreate, db: DbSession, current_user: CurrentUser):
...
def record_rebalance_from_plan(db: DbSession, current_user: CurrentUser):
    plan = compute_rebalance_plan(db, current_user.id)
...
def list_rebalances(
    ...
    records = db.scalars(
        ...
        .where(RebalanceHistory.user_id == current_user.id)
        .order_by(RebalanceHistory.executed_at.desc())
        .limit(limit)
    ).all()
    return records
